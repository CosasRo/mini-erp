...
def render_orcado_realizado():
    ...
    def _destacar_grupos(row):
        ...
        for col_grupo, col_nome in tabela.columns:
            if col_nome == "Orçado" or col_nome == "Previsto":
                estilos.append(base + "color: #f2a5a5")  # vermelho -- orçado
            elif col_nome == "Realizado":
                estilos.append(base + "color: #9cc3f2")  # azul -- realizado
            elif col_nome == "%":
                v = row[(col_grupo, col_nome)]
                cor = "color: #f2a5a5" if v > 1.0 else "color: #8fd6a5"  # vermelho > 100%, verde <= 100%
                estilos.append(base + cor)
            else:
                estilos.append(base)
        return estilos
