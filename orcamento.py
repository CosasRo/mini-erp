...
def render_orcado_realizado():
    ...
    for grupo, leaves in HIERARQUIA.items():
        ...
        for leaf in leaves:
            ...
            if leaf not in CATEGORIAS_FORA_DO_GERAL:
                for i in range(12):
    ...
    rea_soma7 = [0.0] * 12
    for label, nivel, orc, rea in linhas:
        if nivel == 2 and label in CATEGORIAS_FORA_DO_GERAL:
            for i in range(12):
                orc_soma7[i] += orc[i]
                rea_soma7[i] += rea[i]

    # orc_geral/rea_geral (usado nos subtotais de grupo e no total operacional)
    # já exclui as 7 categorias -- o total de tudo é operacional + essas 7
    orc_tudo = [orc_geral[i] + orc_soma7[i] for i in range(12)]
    rea_tudo = [rea_geral[i] + rea_soma7[i] for i in range(12)]

    linha_total_geral_html = _linha_html(
        "Total Geral", orc_tudo, rea_tudo, "font-weight:800; background:#3A2D58; color:#fff;"
    )
    linha_total_operacional_html = _linha_html(
        "Total Operacional", orc_ajustado, rea_ajustado, "font-weight:800; background:#4a3d6b; color:#fff;"
    )
    linha_total_operacional_2_html = _linha_html(
        "Total Operacional II (dividendos, comissões e impostos sobre lucro)",
        orc_soma7, rea_soma7, "font-weight:800; background:#5a4d80; color:#fff;"
    )
    ...
    <table style="width:100%; border-collapse:collapse; font-size:12px; white-space:nowrap;">
        ...
        <tbody>
            ...
            {linha_total_operacional_html}
            {linha_total_operacional_2_html}
