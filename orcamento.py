...
def render_orcado_realizado():
    ...
    def _destacar_grupos(row):
        eh = eh_grupo[row.name]
        base = "font-weight: bold; background-color: #2a2140; " if eh else ""
        borda_topo = "border-top: 3px solid #cbb3f2; " if eh else ""
        estilos = []
        for col_grupo, col_nome in tabela.columns:
            cor = _cor_celula(col_nome, row[(col_grupo, col_nome)]) if col_nome != "Centro de Custo" else None
            borda_esq = "border-left: 3px solid #cbb3f2; " if col_nome == "Orçado" and col_grupo != "" else ""
            estilos.append(base + borda_topo + borda_esq + (f"color: {cor}" if cor else ""))
        return estilos

    estilo = tabela.style.format(formatos).apply(_destacar_grupos, axis=1).hide(axis="index")
    ...
    st.dataframe(estilo, use_container_width=True, height=600)
