...
def render_orcado_realizado():
    ...
    for sub in ("Previsto", "Realizado", "%"):
        ...
        ordem_tela.append(col)
        if sub == "Previsto":
            dados_tela[col] = [sum(l[2]) if l[1] is not None else 0.0 for l in linhas_tela]
        elif sub == "Realizado":
            dados_tela[col] = [sum(l[3]) if l[1] is not None else 0.0 for l in linhas_tela]
        else:
            dados_tela[col] = [
                (sum(l[3]) / sum(l[2])) if (l[1] is not None and sum(l[2])) else 0.0
                for l in linhas_tela
            ]
