...
def render_transacoes_x_bancos():
    ...
    formatos_num = lambda cols: {c: ("{:.0f}" if "Qtd" in c or c == "Qt Tx Geral" or "Diferença Qtd" in c else "{:,.2f}") for c in cols}

    st.markdown("**Cash-in**")
    tab_in = _montar_cashin(sb, data_ini, data_fim)
    st.dataframe(tab_in.style.format(formatos_num(tab_in.columns)), use_container_width=True)

    st.markdown("**Cash-out**")
    tab_out = _montar_cashout(sb, data_ini, data_fim)
    st.dataframe(tab_out.style.format(formatos_num(tab_out.columns)), use_container_width=True)
