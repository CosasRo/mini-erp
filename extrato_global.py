...
def _agregar_interno(_sb, tabela: str, status_alvo: str, coluna_data: str) -> pd.DataFrame:
    resp = _sb.table(tabela).select("*").eq("STATUS", status_alvo).execute()
    df = pd.DataFrame(resp.data)
    if df.empty or coluna_data not in df.columns or "AMOUNT" not in df.columns:
        return pd.DataFrame(columns=["data", "qtd_interno", "valor_interno"])
    ...
    return (
        df.groupby("data")
        .agg(qtd_interno=("AMOUNT", "count"), valor_interno=("AMOUNT", "sum"))
        .reset_index()
    )


def _agregar_banco_asaas(_sb, categoria_alvo: str) -> pd.DataFrame:
    resp = _sb.table("pix_asaas").select("*").execute()
    df = pd.DataFrame(resp.data)
    if df.empty or "Data" not in df.columns or "Valor" not in df.columns:
        return pd.DataFrame(columns=["data", "qtd_banco", "valor_banco"])

    if "categoria" in df.columns:
        df = df[df["categoria"].astype(str).str.strip().str.upper() == categoria_alvo]

    df["data"] = pd.to_datetime(df["Data"], format="%Y-%m-%d", errors="coerce").dt.normalize()
    df["Valor"] = pd.to_numeric(df["Valor"], errors="coerce").fillna(0.0)
    df = df.dropna(subset=["data"])

    return (
        df.groupby("data")
        .agg(qtd_banco=("Valor", "count"), valor_banco=("Valor", "sum"))
        .reset_index()
    )


def _montar_comparacao(_sb, tabela_interna: str, status_alvo: str, coluna_data: str, categoria_asaas: str) -> pd.DataFrame:
    interno = _agregar_interno(_sb, tabela_interna, status_alvo, coluna_data)
    banco = _agregar_banco_asaas(_sb, categoria_asaas)

    comparacao = pd.merge(interno, banco, on="data", how="outer").fillna(0.0)
    if comparacao.empty:
        return comparacao

    comparacao["qtd_interno"] = comparacao["qtd_interno"].astype(int)
    comparacao["qtd_banco"] = comparacao["qtd_banco"].astype(int)
    comparacao["dif_qtd"] = comparacao["qtd_interno"] - comparacao["qtd_banco"]
    comparacao["dif_valor"] = comparacao["valor_interno"] - comparacao["valor_banco"]

    comparacao = comparacao.sort_values("data")
    comparacao["data"] = comparacao["data"].dt.strftime("%d/%m/%Y")
    comparacao = comparacao.rename(columns={
        "data": "Data",
        "qtd_interno": "Qtd interna",
        "qtd_banco": "Qtd banco",
        "valor_interno": "Valor interno",
        "valor_banco": "Valor banco",
        "dif_qtd": "Diferença Qtd",
        "dif_valor": "Diferença Valor",
    })
    return comparacao.set_index("Data")


def render_transacoes_x_bancos():
    st.caption("Por enquanto só Asaas — BB entra quando a classificação (Historico → categoria) estiver pronta em bb_pix/bb_adm.")

    sb = get_supabase()

    st.markdown("**Cash-in**")
    comp_in = _montar_comparacao(sb, "cashin", "PROCESSED", "PAYMENT TIME", "CASH-IN")
    if comp_in.empty:
        st.caption("Sem dados para comparar.")
    else:
        st.dataframe(
            comp_in.style.format({
                "Qtd interna": "{:.0f}", "Qtd banco": "{:.0f}", "Diferença Qtd": "{:.0f}",
                "Valor interno": "{:,.2f}", "Valor banco": "{:,.2f}", "Diferença Valor": "{:,.2f}",
            }),
            use_container_width=True,
        )

    st.markdown("**Cash-out**")
    comp_out = _montar_comparacao(sb, "cashout", "SUCCESSFULLY PROCESSED", "CREATION TIME", "CASH-OUT")
    if comp_out.empty:
        st.caption("Sem dados para comparar.")
    else:
        st.dataframe(
            comp_out.style.format({
                "Qtd interna": "{:.0f}", "Qtd banco": "{:.0f}", "Diferença Qtd": "{:.0f}",
                "Valor interno": "{:,.2f}", "Valor banco": "{:,.2f}", "Diferença Valor": "{:,.2f}",
            }),
            use_container_width=True,
