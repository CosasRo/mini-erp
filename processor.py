...
# ============================================================
# ATUALIZAÇÃO AUTOMÁTICA DA AUDITORIA
# ============================================================

def atualizar_auditoria(sb, df, tipo, ano, mes):
    """
    Após processar CASH-IN ou CASH-OUT, atualiza a tabela auditoria_fluxo
    com os totais IN/OUT do mês por merchant.
    """
    try:
        if tipo == "cashin":
            ...
            col_merchant = "MERCHANT NAME"
            col_valor    = "COMMISSION"
            col_status   = "STATUS"
            status_ok    = {"SUCCESSFULLY PROCESSED"}
        else:
            return

        if col_merchant not in df.columns or col_valor not in df.columns:
            return

        df_valido = df[df[col_status].isin(status_ok)].copy()
        df_valido[col_valor] = pd.to_numeric(df_valido[col_valor], errors="coerce").fillna(0)

        totais = df_valido.groupby(col_merchant)[col_valor].sum().reset_index()
        totais.columns = ["cliente", "valor"]
        ...
        for _, row in totais.iterrows():
            ...
            res = sb.table("auditoria_fluxo").select("*").eq("cliente", cliente).eq("ano", ano).eq("mes", mes).execute()

            if res.data:
                # Atualiza
                reg = res.data[0]
                if tipo == "cashin":
                    in_mes  = float(reg.get("in_mes", 0) or 0) + valor
                    out_mes = float(reg.get("out_mes", 0) or 0)
                else:
                    in_mes  = float(reg.get("in_mes", 0) or 0)
                    out_mes = float(reg.get("out_mes", 0) or 0) + valor

                sdo_ini = float(reg.get("sdo_ini", 0) or 0)
                sdo_mes = sdo_ini + in_mes - out_mes

                # Calcula status
                if sdo_mes >= 0:
                    status = "NORMAL"
                else:
                    perc = abs(sdo_mes) / in_mes * 100 if in_mes > 0 else 100
                    status = "ALERTA" if perc > 10 else "ATENCAO"

                sb.table("auditoria_fluxo").update({
                    "in_mes":  round(in_mes, 2),
                    "out_mes": round(out_mes, 2),
                    "sdo_mes": round(sdo_mes, 2),
                    "status":  status
                ...
                res_ant = sb.table("auditoria_fluxo").select("sdo_mes").eq("cliente", cliente).eq("ano", ano_ant).eq("mes", mes_ant).execute()
                sdo_ini = float(res_ant.data[0]["sdo_mes"]) if res_ant.data else 0.0

                if tipo == "cashin":
                    in_mes = valor; out_mes = 0.0
                else:
                    in_mes = 0.0; out_mes = valor
                ...
                sb.table("auditoria_fluxo").upsert({
                    "cliente": cliente,
                    "ano":     ano,
                    "mes":     mes,
                    "sdo_ini": round(sdo_ini, 2),
                    "in_mes":  round(in_mes, 2),
                    "out_mes": round(out_mes, 2),
                    "sdo_mes": round(sdo_mes, 2),
                    "status":  status
                }, on_conflict="cliente,ano,mes").execute()

    except Exception as e:
        pass  # Silencioso para não interromper o processamento principal
