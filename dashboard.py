...
import pandas as pd
from orcamento import _carregar_orcado_ano, _carregar_realizado_ano, MESES_PT
...
def _gauge_percentual(titulo: str, percentual: float, cor: str):
    pct = max(0.0, min(percentual, 1.5))  # limita visual em 150%
    restante = max(0.0, 1.0 - pct)
    fig = go.Figure(data=[go.Pie(
        values=[pct, restante] if pct <= 1 else [1, 0],
        hole=0.72,
        marker=dict(colors=[cor, "#3d3560"]),
        textinfo="none",
        sort=False,
        direction="clockwise",
    )])
    fig.update_layout(
        annotations=[dict(text=f"<b>{percentual*100:.0f}%</b>", showarrow=False,
                           font=dict(color=BRANCO, size=20))],
        showlegend=False,
        paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
        margin=dict(t=30, b=0, l=0, r=0), height=180,
        title=dict(text=titulo, font=dict(color=CINZA_CLARO, size=12), x=0.5),
    )
    st.plotly_chart(fig, use_container_width=True)
...
def render_drc(sb):
    ...
    with col_ano:
        ano = st.selectbox("Ano", [2025, 2026, 2027], index=1, key="drc_ano")
    with col_mes:
        mes_sel = st.selectbox("Mês", ["Ano todo"] + MESES_PT, index=0, key="drc_mes")
    st.caption("💸 Demonstrativo de custo e despesa (sem receita) — dados de Orçado x Realizado.")
    ...
    if mes_sel == "Ano todo":
        orcado_filtrado = orcado
        realizado_filtrado = realizado
    else:
        num_mes = MESES_PT.index(mes_sel) + 1
        orcado_filtrado = orcado[orcado["mes"].dt.month == num_mes] if not orcado.empty else orcado
        realizado_filtrado = realizado[realizado["mes"].dt.month == num_mes] if not realizado.empty else realizado

    total_orcado = orcado_filtrado["valor_orcado"].sum() if not orcado_filtrado.empty else 0.0
    total_realizado = realizado_filtrado["valor"].sum() if not realizado_filtrado.empty else 0.0
    pct_realizado = (total_realizado / total_orcado) if total_orcado else 0.0

    kpis = st.columns(3)
    with kpis[0]:
        _card_simples("Despesas e Custos (Realizado)", f"R$ {total_realizado:,.0f}")
    with kpis[1]:
        _card_simples("Orçado Total", f"R$ {total_orcado:,.0f}")
    with kpis[2]:
        _card_simples("% Realizado", f"{pct_realizado*100:.0f}%")
    ...
    with col_bar:
        orcado_mes = [0.0] * 12
        realizado_mes = [0.0] * 12
        if not orcado.empty:
            for m in range(1, 13):
                orcado_mes[m - 1] = orcado.loc[orcado["mes"].dt.month == m, "valor_orcado"].sum()
        if not realizado.empty:
            for m in range(1, 13):
                realizado_mes[m - 1] = realizado.loc[realizado["mes"].dt.month == m, "valor"].sum()

        texto_orcado = [f"R$ {v:,.0f}" for v in orcado_mes]
        texto_realizado = []
        for o, r in zip(orcado_mes, realizado_mes):
            dif_pct = ((r / o) - 1) * 100 if o else 0.0
            sinal = "+" if dif_pct >= 0 else ""
            texto_realizado.append(f"R$ {r:,.0f}<br>({sinal}{dif_pct:.0f}%)")

        fig = go.Figure()
        fig.add_bar(x=MESES_PT, y=orcado_mes, name="Orçado", marker_color=ROXO_CLARO,
                    text=texto_orcado, textposition="outside", textfont=dict(size=9, color=CINZA_CLARO))
        fig.add_bar(x=MESES_PT, y=realizado_mes, name="Realizado", marker_color=VERMELHO,
                    text=texto_realizado, textposition="outside", textfont=dict(size=9, color=CINZA_CLARO))
        fig.update_layout(
            title=dict(text="Orçado x Realizado | R$", font=dict(color=BRANCO, size=13)),
            barmode="group",
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color=CINZA_CLARO, size=10),
            legend=dict(orientation="h", y=1.15, font=dict(color=CINZA_CLARO)),
    ...
    with col_gauge:
        _gauge_percentual("Previsto x Realizado", pct_realizado, VERMELHO if pct_realizado > 1 else VERDE)

    st.markdown("<br>", unsafe_allow_html=True)
    titulo_despesas = "Despesas/Custos" + (f" — {mes_sel}" if mes_sel != "Ano todo" else " — ano todo")
    if not realizado_filtrado.empty:
        todos = realizado_filtrado.groupby("centro_custo")["valor"].sum().sort_values(ascending=True)
        altura = max(340, 22 * len(todos))
        fig = go.Figure(go.Bar(
            x=todos.values, y=todos.index, orientation="h",
            marker_color=VERMELHO,
            text=[f"{v:,.0f}" for v in todos.values], textposition="outside",
            textfont=dict(color=CINZA_CLARO, size=9),
        ))
        fig.update_layout(
            title=dict(text=titulo_despesas, font=dict(color=BRANCO, size=13)),
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color=CINZA_CLARO, size=9),
            margin=dict(t=40, b=10, l=10, r=60), height=altura,
            xaxis=dict(gridcolor="#3d3560"), yaxis=dict(gridcolor="#3d3560"),
        )
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.caption(f"Sem compromissos lançados para {mes_sel if mes_sel != 'Ano todo' else 'este ano'} ainda.")
