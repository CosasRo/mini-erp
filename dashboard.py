...
def render_drc(sb):
    ...
    total_orcado = orcado["valor_orcado"].sum() if not orcado.empty else 0.0
    total_realizado = realizado["valor"].sum() if not realizado.empty else 0.0
    ...
    with kpis[2]:
        cor_pct = VERMELHO if pct_realizado > 1.0 else VERDE
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    if not realizado.empty:
        top10 = realizado.groupby("centro_custo")["valor"].sum().sort_values(ascending=True).tail(10)
        fig = go.Figure(go.Bar(
            x=top10.values, y=top10.index, orientation="h",
            marker_color=VERMELHO,
            text=[f"{v:,.0f}" for v in top10.values], textposition="outside",
            textfont=dict(color=CINZA_CLARO, size=10),
        ))
        fig.update_layout(
            title=dict(text="Top 10 Despesas/Custos", font=dict(color=BRANCO, size=13)),
            paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
            font=dict(color=CINZA_CLARO, size=10),
        ...
        st.caption("Sem compromissos lançados ainda para montar o Top 10.")
