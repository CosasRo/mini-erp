...
def app_principal():
    ...
    # JURÍDICO / LEGAL
        ...
        st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
        st.info("🚧 Em desenvolvimento — Gestão de contratos, processos e documentos jurídicos.")
    ...
    # CONTÁBIL / TRIBUTÁRIO
        ...
        st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
        st.info("🚧 Em desenvolvimento — Controle contábil, DRE, balanço e obrigações tributárias.")
        ...
        if not df_cli.empty:
            ...
            if busca:
                mask = (
                    ...
                    df_show["CPF/CNPJ"].astype(str).str.contains(busca, case=False, na=False)
                )
            ...
            cols_show = ["NOME FANTASIA","RAZAO SOCIAL","CPF/CNPJ","TELEFONE","EMAIL","PLATAFORMA","SEGMENTO","CIDADE","ESTADO","ATIVO"]
            cols_show = [c for c in cols_show if c in df_show.columns]

            # Formata CPF/CNPJ para exibição
            df_display = df_show[cols_show].copy()
            df_display["CPF/CNPJ"] = df_display["CPF/CNPJ"].apply(formatar_cpf_cnpj)
            ...
            with col_s1:
                if st.button("💾 Salvar Alterações", type="primary", key="save_cli", use_container_width=True):
                    ...
                    for i, row in edited_cli.iterrows():
                        cnpj_limpo = ''.join(c for c in str(row["CPF/CNPJ"]) if c.isdigit())
                        try:
                            ...
                            }).eq("CPF/CNPJ", cnpj_limpo).execute()
                        except Exception:
                            erros_save += 1
                    if erros_save == 0:
            ...
            with col_s2:
                if st.button("🗑️ Inativar Selecionados", key="del_cli", use_container_width=True):
                    inativos = edited_cli[edited_cli["ATIVO"] == "NÃO"]["CPF/CNPJ"].tolist()
                    for cnpj in inativos:
                        cnpj_limpo = ''.join(c for c in str(cnpj) if c.isdigit())
                        sb.table("clientes").update({"ATIVO": "NÃO"}).eq("CPF/CNPJ", cnpj_limpo).execute()
