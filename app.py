    ...
    # EMISSÃO DE NOTA FISCAL
                ...
                with st.spinner("Buscando transações..."):
                    try:
                        ...
                        if not fee_in.empty and not fee_out.empty:
                            df_nf = fee_in.merge(fee_out, on="MERCHANT", how="outer").fillna(0)
                        elif not fee_in.empty:
                            df_nf = fee_in.copy(); df_nf["FEE_OUT"] = 0
                        elif not fee_out.empty:
                            df_nf = fee_out.copy(); df_nf["FEE_IN"] = 0
                        else:
                        ...
                        if not df_nf.empty:
                            ...
                            # Busca CNPJ dos clientes
                            clientes_res = sb.table("clientes").select("NOME FANTASIA,RAZAO SOCIAL,CPFCNPJ").execute()
                            df_cli = pd.DataFrame(clientes_res.data)
                            if not df_cli.empty:
                                df_cli["NOME FANTASIA"] = df_cli["NOME FANTASIA"].astype(str).str.strip().str.upper()
                                df_nf["MERCHANT_UP"] = df_nf["MERCHANT"].astype(str).str.strip().str.upper()
                                df_nf = df_nf.merge(
                                    df_cli[["NOME FANTASIA","RAZAO SOCIAL","CPFCNPJ"]].rename(columns={"NOME FANTASIA":"MERCHANT_UP"}),
                                    on="MERCHANT_UP", how="left"
                                )
                            ...
                            cols_show = ["Razão Social / Nome","Razão Social Completa","CPF ou CNPJ","FEE IN","FEE OUT","Valor"]
        ...
        with aba_nf[1]:
            ...
            if df_pend.empty:
                ...
                with col_c1:
                    pfx_file = st.file_uploader("📎 Certificado Digital (.pfx)", type=["pfx"], key="pfx_upload")
                    senha_pfx = st.text_input("🔑 Senha do Certificado", type="password", key="senha_pfx")
                with col_c2:
                    descricao_nf = st.text_area("📝 Descrição da NF", height=100, key="desc_nf",
                        value=f"Prestacao de servicos de intermediacao de pagamentos. Cash-In e Cash-Out. Valor debitado diretamente no repasse, nao havendo necessidade de pagamento do mes")
                    st.caption("⚠️ A emissão conecta diretamente com a Prefeitura de SP")

                if st.button("🚀 EMITIR NOTAS FISCAIS", type="primary", use_container_width=True, key="btn_emitir"):
                    if not pfx_file or not senha_pfx:
                        st.error("❌ Envie o certificado e a senha antes de emitir!")
                    else:
                        try:
