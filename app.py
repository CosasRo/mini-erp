            ...
            if st.button("📦 Gerar Pacote ZIP", type="primary", disabled=len(arquivos_sel)==0):
                try:
                    ...
                    with zipfile.ZipFile(buf_zip, "w") as zf:
                        ...
                        manifesto = f"Pacote Contabilidade — {MESES_NOMES[pac_mes-1]}/{pac_ano}
"
                        manifesto += f"Gerado em: {pd.Timestamp.now().strftime('%d/%m/%Y %H:%M')}
"
                        manifesto += f"Por: {st.session_state.get('nome','')}
