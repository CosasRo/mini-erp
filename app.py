                ...
                for _, row in df_fech.iterrows():
                    ...
                    for i, col_key in enumerate(COLUNAS_FECHAMENTO):
                        ...
                        if val_atual not in STATUS_OPTS:
                            val_atual = ""
                        idx = STATUS_OPTS.index(val_atual)

                        # Selectbox com cor de fundo via CSS dinâmico
                        if val_atual == "REALIZADO":
                            cor_bg = "#d4edda"; cor_txt = "#1a7a3a"
                        elif val_atual == "PENDENTE":
                            cor_bg = "#fff3cd"; cor_txt = "#856404"
                        elif val_atual == "ENVIADO":
                            cor_bg = "#bae6fd"; cor_txt = "#0c4a6e"
                        else:
                            cor_bg = "#2a2a3a"; cor_txt = "#888"

                        st.markdown(f"""
                        <style>
                        div[data-testid="stSelectbox"][key="fech_{ano_sel}_{row['mes']}_{col_key}"] > div > div {{
