...
<style>
    ...
    .stButton > button[kind="primary"]:hover {{
        background-color: #d4a832 !important;
    }}
    ...
    /* Login card */
    .login-card {{
        background: {ROXO_ESCURO};
        border-radius: 16px;
        padding: 40px;
        max-width: 420px;
        margin: 0 auto;
        box-shadow: 0 8px 32px rgba(58,45,88,0.3);
    }}

    /* Divider colorido */
    .divider {{
        height: 3px;
        background: linear-gradient(90deg, {ROXO_MEDIO}, {AMARELO});
        border-radius: 2px;
        margin: 16px 0;
    }}
</style>
...
# ============================================================
# AUTENTICAÇÃO - Usuários hardcoded por enquanto
# (depois migrar para tabela no Supabase)
# ============================================================
USUARIOS = {
    "admin": {
        "senha": hash_senha("pagoexpress2024"),
        "nome": "Administrador",
        "perfil": "admin"
    },
    "financeiro": {
        "senha": hash_senha("fin2024"),
        "nome": "Financeiro",
        "perfil": "usuario"
    },
}
...
def logout():
    for key in ["logado", "usuario", "nome", "perfil"]:
    ...
    with col2:
        ...
        <div style="text-align:center; margin-bottom: 32px;">
            <div style="
                ...
                border-radius: 20px;
                padding: 32px 40px;
                box-shadow: 0 8px 32px rgba(58,45,88,0.4);
            ">
                <div style="font-size: 3rem;">💳</div>
                <div style="color: {AMARELO}; font-size: 1.8rem; font-weight: 700; font-family: Poppins;">
                    Pago Express
                </div>
                <div style="color: {CINZA_CLARO}; font-size: 0.85rem; margin-top: 4px;">
                    Sistema de Gestão Financeira
                </div>
                <div style="height: 3px; background: linear-gradient(90deg, {ROXO_MEDIO}, {AMARELO}); border-radius: 2px; margin: 20px 0;"></div>
                <div style="color: {BRANCO}; font-size: 1rem; font-weight: 500; margin-bottom: 8px;">
                    Acesso ao Sistema
                </div>
            </div>
        </div>
        ...
        st.markdown(f"""
        <div style="text-align:center; margin-top: 24px; color: {CINZA_CLARO}; font-size: 0.75rem;">
            © 2024 Pago Express. Todos os direitos reservados.
        </div>
        """, unsafe_allow_html=True)
...
def app_principal():
    ...
    with st.sidebar:
        ...
        <div style="background: {ROXO_MEDIO}44; border-radius: 8px; padding: 10px 12px; margin-bottom: 20px;">
            ...
            <div style="color: {CINZA_CLARO}; font-size: 0.7rem; text-transform: uppercase;">{perfil}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    if pagina == "📊 Dashboard":
        ...
        with col_a:
            try:
                ...
                st.bar_chart(contagem.set_index("Status"), color=ROXO_MEDIO)
            except Exception as e:
                st.error(f"Erro: {e}")
        ...
        with col_b:
            try:
                ...
                st.bar_chart(contagem2.set_index("Status"), color=AMARELO)
            except Exception as e:
                st.error(f"Erro: {e}")
        ...
        try:
            ...
            res_m = sb.table("cashin").select("MERCHANT NAME,FEE,STATUS").eq("STATUS", "PROCESSED").execute()
            df_m = pd.DataFrame(res_m.data)
            df_m["FEE"] = pd.to_numeric(df_m["FEE"], errors="coerce")
            top = df_m.groupby("MERCHANT NAME")["FEE"].sum().sort_values(ascending=False).head(10).reset_index()
    ...
    # ============================================================
        ...
        st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
        st.markdown("Faça o upload dos arquivos brutos. O sistema identifica automaticamente o tipo, processa e envia ao banco.")
        ...
        if arquivos:
            ...
            for nome, (arq, tipo) in mapa.items():
                ...
                dados_tabela.append({"Arquivo": nome, "Tipo": status})
    ...
    # ============================================================
        ...
        st.info("ℹ️ Os usuários abaixo estão configurados no sistema. Em breve será possível gerenciá-los diretamente aqui.")

        usuarios_df = pd.DataFrame([
            {"Usuário": k, "Nome": v["nome"], "Perfil": v["perfil"]}
        ...
        if st.button("➕ Adicionar Usuário", type="primary"):
            if novo_usuario and novo_nome and nova_senha:
                USUARIOS[novo_usuario.lower()] = {
                    "senha": hash_senha(nova_senha),
                    "nome": novo_nome,
                    "perfil": novo_perfil
                }
                st.success(f"✅ Usuário '{novo_usuario}' criado! (válido até reiniciar o app)")
                st.info("💡 Para tornar permanente, os usuários precisarão ser salvos no banco de dados — melhoria já planejada!")
            else:
                st.warning("⚠️ Preencha todos os campos")
