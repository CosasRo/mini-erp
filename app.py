...
from orcamento import render_orcamento
...
def app_principal():
    ...
    with st.sidebar:
        ...
        if perfil == "admin":
            ...
            paginas.append("💰 Orçado x Realizado")
        ...
        pagina = st.radio("", sorted(paginas, key=lambda x: x.split(" ", 1)[1]), label_visibility="collapsed")
    ...
    # ORÇADO X REALIZADO
    elif pagina == "💰 Orçado x Realizado" and perfil == "admin":
        st.markdown(f'<h1 style="color:{ROXO_ESCURO};">💰 Orçado x Realizado</h1>', unsafe_allow_html=True)
        st.markdown('<div class="divider"></div>', unsafe_allow_html=True)
        render_orcamento()
