...
def mostrar_historico_uploads(sb):
    try:
        ...
        for data, grupos in sorted(por_data.items(), reverse=True):
            ...
            with col_t:
                st.markdown('<div style="color:#C9C7C1;font-size:0.75rem;margin-bottom:6px;">📁 Arquivos de Transações</div>', unsafe_allow_html=True)
                if grupos["transacao"]:
            ...
            with col_p:
                st.markdown('<div style="color:#C9C7C1;font-size:0.75rem;margin-bottom:6px;">📊 Extrato PIX Asaas</div>', unsafe_allow_html=True)
                if grupos["pix_asaas"]:
...
def app_principal():
    ...
    with st.sidebar:
        ...
        if perfil == "admin": paginas.append("👥 Usuários")
                    ...
                    with st.expander(f"📄 {nome_arq}", expanded=True):
                        try:
                            with st.spinner("Processando..."):
                                if tipo == "cashin": df, tabela, chave = processar_cashin(arq, asaas_taxa_lookup, asaas_valor_lookup)
                                elif tipo == "cashout": df, tabela, chave = processar_cashout(arq, asaas_taxa_lookup, asaas_valor_lookup)
    ...
    # CONSULTAR
        ...
        try:
            with st.spinner("Carregando dados..."):
                df_raw = pd.DataFrame(sb.table(tipo).select("*").limit(limite).execute().data)
        except Exception as e:
        ...
        if not df_raw.empty:
            ...
            with col_f1:
                ...
                if data_col in df_raw.columns:
                    datas_validas = pd.to_datetime(df_raw[data_col], dayfirst=True, errors="coerce").dropna()
            ...
            with col_f4:
                ...
                if merch_col in df_raw.columns:
            ...
            with col_f5:
                if "CONCILIAÇÃO" in df_raw.columns:
                    conc_opts = ["Todos"] + sorted(df_raw["CONCILIAÇÃO"].dropna().unique().tolist())
                    conc_sel = st.selectbox("Conciliação", conc_opts, key="cons_conc")
                else:
                    conc_sel = "Todos"
            with col_f6:
            ...
            if d_ini and d_fim and data_col in df.columns:
                datas = pd.to_datetime(df[data_col], dayfirst=True, errors="coerce").dt.date
            ...
            if conc_sel != "Todos" and "CONCILIAÇÃO" in df.columns:
                df = df[df["CONCILIAÇÃO"].astype(str) == conc_sel]
            ...
            if tipo in ["cashin", "cashout"] and "END TO END ID" in df.columns:
                try:
                    end_to_end_ids = df["END TO END ID"].dropna().unique().tolist()
                    if end_to_end_ids:
                        res_pix = sb.table("pix_asaas").select(
                            "Identificador fim a fim,Valor,Valor da taxa,Situação,Tipo,Data"
                        ).in_("Identificador fim a fim", end_to_end_ids[:500]).execute()
                        df_pix_banco = pd.DataFrame(res_pix.data)
                        if not df_pix_banco.empty:
                            df_pix_banco = df_pix_banco.rename(columns={
                                "Identificador fim a fim": "END TO END ID",
                                "Valor": "BANCO_VALOR",
                            ...
                            df = df.merge(df_pix_banco, on="END TO END ID", how="left")
                except Exception:
                    pass
