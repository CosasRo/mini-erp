    ...
    # CONSULTAR
        ...
        if not df_raw.empty:
            ...
            if tipo in ["cashin", "cashout"] and "END TO END ID" in df.columns:
                try:
                    # Busca TODOS os registros do pix_asaas disponíveis
                    ...
                    if not df_pix_banco.empty:
                        ...
                        n_conc = df["BANCO_VALOR"].notna().sum()
                        if n_conc > 0:
                            st.info(f"📊 {n_conc} transações cruzadas com extrato Asaas")
                        else:
                            st.warning("⚠️ Nenhuma transação cruzada com extrato Asaas — suba o arquivo PIX Asaas do mesmo período na aba Upload")
                except Exception as e:
