...
def criar_usuario(sb, usuario, nome, email, senha, perfil):
    try:
        sb.table("usuarios").insert({
            "usuario": usuario.lower(), "nome": nome, "email": email,
            "senha_hash": hash_senha(senha), "perfil": perfil, "ativo": True
        }).execute()
...
def registrar_log(sb, arquivo, tipo, total, enviados, erros, usuario, categoria="transacao"):
    ...
    try:
        sb.table("log_uploads").insert({
            "arquivo": arquivo, "tipo": tipo, "total": total,
...
def mostrar_historico_uploads(sb):
    try:
        res = sb.table("log_uploads").select("*").order("data_upload", desc=True).limit(40).execute()
        logs = res.data
        if not logs:
        ...
        TIPO_EMOJI = {"cashin": "📥", "cashout": "📤", "pagamentos": "💳", "cartao": "💰", "pix_asaas": "📊"}
        ...
        def card_log(log):
            ...
            <div style="display:flex; align-items:center; justify-content:space-between;
                background:#1e1e2e; border:1px solid #3A2D58;
                border-left:4px solid {cfg['cor']}; border-radius:8px;
                padding:8px 12px; margin-bottom:6px;">
                <div style="display:flex; align-items:center; gap:10px; overflow:hidden;">
                    <span style="font-size:1.1rem; flex-shrink:0;">{tipo_emoji}</span>
                    <div style="overflow:hidden;">
                        <div style="color:#FFFFFF; font-size:0.78rem; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">{arquivo}</div>
                        <div style="color:#C9C7C1; font-size:0.68rem;">{data_str} | {tipo} | {enviados} linhas | {usuario}</div>
                    </div>
                </div>
                <div style="background:{cfg['fundo']}; color:{cfg['cor']}; border-radius:20px;
                    padding:3px 10px; font-size:0.7rem; font-weight:600; white-space:nowrap; margin-left:8px;">
                    {cfg['emoji']} {cfg['texto']}
                </div>
            </div>"""
        ...
        por_data = defaultdict(lambda: {"transacao": [], "pix_asaas": []})
        for log in logs:
            data = log.get("data_upload", "")[:10] if log.get("data_upload") else "Sem data"
            cat = log.get("categoria", "transacao")
            por_data[data][cat].append(log)
        ...
        for data, grupos in sorted(por_data.items(), reverse=True):
            ...
            st.markdown(f'<div style="color:#ECBD42; font-size:0.9rem; font-weight:600; margin:16px 0 8px 0;">📅 {data_fmt}</div>', unsafe_allow_html=True)

            col_t, col_p = st.columns(2)
            with col_t:
                st.markdown(f'<div style="color:#C9C7C1; font-size:0.75rem; margin-bottom:6px;">📁 Arquivos de Transações</div>', unsafe_allow_html=True)
                if grupos["transacao"]:
                    for log in grupos["transacao"]:
                        st.markdown(card_log(log), unsafe_allow_html=True)
                else:
                    st.markdown('<div style="color:#555; font-size:0.75rem; padding:8px;">— Nenhum arquivo</div>', unsafe_allow_html=True)

            with col_p:
                st.markdown(f'<div style="color:#C9C7C1; font-size:0.75rem; margin-bottom:6px;">📊 Extrato PIX Asaas</div>', unsafe_allow_html=True)
                if grupos["pix_asaas"]:
                    for log in grupos["pix_asaas"]:
                        st.markdown(card_log(log), unsafe_allow_html=True)
                else:
                    st.markdown('<div style="color:#555; font-size:0.75rem; padding:8px;">— Nenhum arquivo</div>', unsafe_allow_html=True)
...
def carregar_pix_asaas(arquivo_pix):
    ...
    return taxa_lookup, valor_lookup, df
        ...
        <div style="text-align:center; margin-bottom:32px;">
            <div style="background:{ROXO_ESCURO}; border-radius:20px; padding:32px 40px; box-shadow:0 8px 32px rgba(58,45,88,0.4);">
                <div style="margin-bottom:8px;">
                    <img src="{LOGO_URL}" width="180" style="filter:brightness(0) invert(1);"/>
                </div>
...
def app_principal():
    ...
    # UPLOAD
        ...
        # Mostra detecção dos arquivos xlsx
        mapa = {}
        if arquivos:
            st.markdown("---")
        ...
        # Botão aparece se tiver qualquer arquivo (xlsx OU csv)
        if arquivos or arquivo_pix:
            st.markdown("---")
            if st.button("🚀 Processar e Enviar ao Banco", type="primary", use_container_width=True):
                ...
                if arquivo_pix:
                    ...
                    with st.expander(f"📊 {arquivo_pix.name}", expanded=True):
                        ...
                        enviados_pix, erros_pix = upload_supabase(df_pix, "pix_asaas", "Identificador fim a fim", sb)
                        if erros_pix == 0:
                            st.success(f"✅ {enviados_pix} registros PIX Asaas enviados!")
                        else:
                            st.warning(f"⚠️ {enviados_pix} enviados | {erros_pix} erros")
                        registrar_log(sb, arquivo_pix.name, "pix_asaas", len(df_pix), enviados_pix, erros_pix, st.session_state.get("usuario", ""), categoria="pix_asaas")
