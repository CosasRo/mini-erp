...
def app_principal():
    ...
    # CONSULTAR
        ...
        with col1: tipo = st.selectbox("Tabela", ["cashin","cashout","pagamentos","cartao"])
        with col2: limite = st.slider("Quantidade de registros", 10, 1000, 100)
        if st.button("🔍 Consultar", type="primary"):
            try:
                df = pd.DataFrame(sb.table(tipo).select("*").limit(limite).execute().data)
                st.success(f"✅ {len(df)} registros")
                st.dataframe(df, use_container_width=True)
                buf = io.BytesIO()
                with pd.ExcelWriter(buf, engine="xlsxwriter") as w: df.to_excel(w, index=False)
                st.download_button("📥 Baixar Excel", buf.getvalue(), file_name=f"{tipo}_consulta.xlsx",
